...
pub mod ERC20 {
    ...
    #[constructor]
    fn constructor(
        ref self: ContractState, name: felt252, symbol: felt252, decimals: u8, initial_supply: u256
    ) {
    ...
    // self._mint(caller, initial_supply);
    }
    ...
    impl ERC20 of super::IERC20<ContractState> {
        ...
        fn transfer_from(
            ...
            if current_allowance != BoundedInt::max() {
                ...
                self.emit(Approval { owner: sender, spender: caller, value: amount });
            }
            self._balances.write(sender, self._balances.read(sender) - amount);
            self._balances.write(recipient, self._balances.read(recipient) + amount);
            self.emit(Transfer { from: sender, to: recipient, value: amount });
    ...
    #[generate_trait]
    impl InternalImpl<TState> of InternalTrait<TState> {
        fn _transfer(
            ref self: ContractState,
            sender: ContractAddress,
            recipient: ContractAddress,
